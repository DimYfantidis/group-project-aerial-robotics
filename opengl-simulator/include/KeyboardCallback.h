...
bool arrow_right_loaded;
bool arrow_left_loaded;
...
void initModuleKeyboardCallback(void)
{
    ...
    arrow_left_loaded = false;
    arrow_right_loaded = false;
...
void callbackSpecialKeyboard(int key, int x, int y)
{
    ...
    if ((current_ms = getAbsoluteTimeMillis()) - cooldown_ts >= cooldown_ms)
    {
        ...
        if (key == GLUT_KEY_LEFT)
            arrow_left_loaded = true;
        
        if (key == GLUT_KEY_RIGHT)
            arrow_right_loaded = true;
    }
}

void callbackSpecialUpKeyboard(int key, int x, int y)
{
    static const uint64_t cooldown_ms = 100;
